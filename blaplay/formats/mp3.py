...
def isascii(string): return ((len(string) == 0) or (ord(max(string)) < 128))
...
class Mp3(BlaTrack):
    ...
    def __parse_id3v1(self, string):
        ...
        except StructError: return frames

        if tag != "TAG": return frames

        def fix(s): return s.split("\x00")[0].strip().decode("latin1")
        title, artist, album, year, comment = map(
                fix, [title, artist, album, year, comment])

        if title: frames["TIT2"] = id3.TIT2(encoding=3, text=title)
        if artist: frames["TPE1"] = id3.TPE1(encoding=3, text=[artist])
        if album: frames["TALB"] = id3.TALB(encoding=3, text=album)
        if year: frames["TDRC"] = id3.TDRC(encoding=3, text=year)
        if comment:
            frames["COMM"] = id3.COMM(encoding=3, lang="eng",
                    desc="ID3v1 Comment", text=comment)
    ...
    def _save(self):
        ...
        try:
            for identifier in self._deleted_tags:
                ...
                except KeyError: tag = "TXXX:%s" % identifier
                delall(tag)
        ...
        for identifier in self.keys_tags():
            ...
            try:
                ...
                tag = "TXXX"
                if identifier == ALBUM_ARTIST: identifier = "ALBUM ARTIST"
                else: identifier = identifier.upper()
